...
def make_env():
    # Variante "shaped" : on applique le même reward shaping que pour PPO
    # afin d'isoler la contribution du shaping vs celle de l'algorithme.
    env = TripleAcrobotEnv(render_mode=None)
    env = TripleAcrobotRewardWrapper(env)
    env = TimeLimit(env, max_episode_steps=1000)
    return env

# Vectorisation sur 4 cœurs
...
model = DQN(
    ...
    batch_size=128,
    exploration_fraction=0.5,     # Décroissance du hasard sur les premiers 50%
...
print("Début de l'apprentissage DQN (variante SHAPED) ...")
# 1M timesteps suffisent pour montrer la tendance, et permettent une
# comparaison équitable avec PPO (1M) et REINFORCE (~1-2M).
model.learn(total_timesteps=1000000, callback=eval_callback, tb_log_name="DQN_Shaped_TripleAcrobot")
...
# Sanity check : liste tout ce qui est dans /kaggle/working/
