"""PPO + Potential-Based Reward Shaping on the Triple Acrobot — BIGNET ablation.

Identical to kaggle/ppo_pbrs_kaggle.py except for ONE change: the policy/value
MLP is widened from the SB3 default [64, 64] to [256, 256]. Used to test whether
a larger network changes PPO+PBRS performance (suggestion of "réseaux spécifiques").

Outputs to /kaggle/working/results_ppo_pbrs_bignet/{models,logs,tensorboard_logs}/.
...
RUN_NAME = "ppo_pbrs_bignet"
WORK = "/kaggle/working/results_ppo_pbrs_bignet"
...
# Seule différence avec ppo_pbrs_kaggle.py : MLP élargi de [64, 64] à [256, 256]
# (pi et vf). Activation tanh conservée (défaut SB3 pour PPO).
...
print("Starting PPO training (PBRS + bignet [256, 256])...")
model.learn(
    total_timesteps=1_000_000,
    callback=eval_callback,
    tb_log_name="PPO_PBRS_BigNet_TripleAcrobot",
)
print("Done.")
model.save(f"{WORK}/models/ppo_pbrs_bignet_triple_acrobot_final")
