"""DQN on the Triple Acrobot, sparse reward + LINEAR LEARNING-RATE DECAY — Kaggle GPU notebook.

Variante du baseline `dqn_sparse_kaggle.py` : seul changement, le learning rate
est un schedule linéaire qui décroît de 1e-3 (début) à 0.0 (fin) sur la durée
totale de l'entraînement. Tout le reste (epsilon-decay 1.0->0.05 sur 50%,
buffer 100k, batch 128, 1M timesteps, TimeLimit 1000, env sparse) est identique
au baseline pour isoler l'effet du LR decay.
...
RUN_NAME = "dqn_sparse_lrdecay"
WORK = "/kaggle/working"
RESULTS_DIR = f"{WORK}/results_{RUN_NAME}"
os.makedirs(f"{RESULTS_DIR}/models", exist_ok=True)
os.makedirs(f"{RESULTS_DIR}/logs", exist_ok=True)
os.makedirs(f"{RESULTS_DIR}/tensorboard_logs", exist_ok=True)
...
def linear_schedule(initial_value: float):
    """SB3 LR schedule: progress_remaining goes 1.0 (start) -> 0.0 (end).
    Returns initial_value * progress_remaining, so lr decays linearly to 0.
    """
...
eval_callback = EvalCallback(
    eval_env,
    best_model_save_path=f"{RESULTS_DIR}/models/",
    log_path=f"{RESULTS_DIR}/logs/",
    eval_freq=10000,
    deterministic=True,
    render=False,
)

# Hyperparameters: identical to dqn_sparse_kaggle.py baseline, EXCEPT learning_rate
# is now a linear schedule 1e-3 -> 0.0 over the full training horizon.
model = DQN(
    "MlpPolicy",
    vec_env,
    ...
    verbose=1,
    tensorboard_log=f"{RESULTS_DIR}/tensorboard_logs/",
    device="cuda",
)

print("Starting DQN training (sparse reward, linear LR decay 1e-3 -> 0)...")
model.learn(
    total_timesteps=1_000_000,
    callback=eval_callback,
    tb_log_name="DQN_Sparse_LRdecay_TripleAcrobot",
)
print("Done.")
model.save(f"{RESULTS_DIR}/models/dqn_sparse_lrdecay_triple_acrobot_final")
...
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for sub in ["models", "logs", "tensorboard_logs"]:
        sub_dir = f"{RESULTS_DIR}/{sub}"
        if not os.path.isdir(sub_dir):
            continue
        for root, _, files in os.walk(sub_dir):
            for f in files:
                full = os.path.join(root, f)
                zf.write(full, arcname=os.path.relpath(full, RESULTS_DIR))
