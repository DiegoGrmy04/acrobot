...
# ==========================================
# 3. WRAPPER "POTENTIAL-BASED REWARD SHAPING" (PBRS)
# ==========================================
# Référence : Ng, Harada & Russell (1999), "Policy Invariance Under
# Reward Transformations", ICML. Théorème : si F(s, a, s') = γ·Φ(s') - Φ(s),
# alors la politique optimale est invariante par rapport à l'env original.
#
# Choix de potentiel : Φ(s) = hauteur(s) = -cos θ1 - cos(θ1+θ2) - cos(θ1+θ2+θ3)
# (= la même quantité que celle utilisée pour le shaping naïf, mais formulée
#  comme une *différence de potentiels* au lieu d'une récompense directe).
#
# IMPORTANT : pas de bonus terminal +100, car cela violerait la forme PBRS.
# La récompense terminale (0 vs -1) reste celle de l'env original.

class TripleAcrobotPBRSWrapper(gym.Wrapper):
    GAMMA = 0.99  # doit être identique à celui du modèle PPO (cf. ci-dessous)

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        # On stocke Φ(s_0) pour le calcul de F au prochain step.
        self._prev_phi = self._potential(self.unwrapped.state)
        return obs, info

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)
        s_next = self.unwrapped.state
        phi_next = self._potential(s_next)
    ...
    @staticmethod
    def _potential(s):
        # hauteur de l'extrémité du 3e segment, dans [-3, +3]
...
def make_env():
    # Variante "PBRS" : reward shaping POTENTIAL-BASED conforme au théorème
    # de Ng-Harada-Russell (1999). Préserve théoriquement la politique optimale.
...
print("Début de l'apprentissage PPO avec PBRS-conforme (1 million de steps)...")
model.learn(total_timesteps=1000000, callback=eval_callback, tb_log_name="PPO_PBRS_TripleAcrobot")
